...
const SignIn: FC<IProps> = ({ onLogin }) => {
  ...
  const onSubmitLogin = () => {
    ...
    signInWithEmailAndPassword(auth, email, password)
      .then((userCredential) => {
        userCredential.user.getIdToken().then((user) => {
  ...
  return (
    ...
    transition={{ duration: 0.2 }}
     className="w-11/12  py-2 mx-auto md:w-3/5 lg:w-2/5">
      <h1 className="text-4xl font-bold text-center">Login to continue</h1>
      <p className="py-4 text-center">
        New to us?
        <button onClick={onLogin} className="pl-2 link link-primary">
          Sign UP
        </button>
      </p>
      <form className="mt-4 space-y-4" onSubmit={handleSubmit(onSubmitLogin)}>
